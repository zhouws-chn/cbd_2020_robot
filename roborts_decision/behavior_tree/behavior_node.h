...
class PreconditionNode: public DecoratorNode{
 public:
  typedef std::shared_ptr<PreconditionNode> Ptr;
  /**
 ...
 protected:
  ...
  virtual bool Precondition(){
    if(precondition_function_){
     // ROS_INFO("precondition_function_ run");
  ...
  virtual BehaviorState Update(){
    // ROS_INFO("precondition_function_ Update");
    if(child_node_ptr_ == nullptr){
   //   ROS_INFO("precondition_child_node_ptr_ == nullptr");
      return BehaviorState::SUCCESS;
    }
    // Reevaluation
    if(Reevaluation()){
     // ROS_INFO("Update Reevaluation");
...
class SelectorNode: public CompositeNode{
 public:
 //太重要了
  typedef std::shared_ptr<SelectorNode> Ptr;
  /**
